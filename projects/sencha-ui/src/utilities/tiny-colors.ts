...
export class tinycolor {
  ...
  public static fromRatio(color, opts?) {
  ...
  public static equals(color1, color2) {
    ...
    return (
      new tinycolor(color1).toRgbString() ===
      new tinycolor(color2).toRgbString()
    );
  }
  ...
  public static mix(color1, color2, amount) {
  ...
  public static readability(color1, color2) {
  ...
  public static isReadable(color1, color2, wcag2) {
  ...
  public static mostReadable(baseColor, colorList, args) {
    var bestColor = null;
    var bestScore = 0;
    var readability;
  ...
  private _applyModification(fn, args) {
...
function bound01(n: string, max: number) {
  if (isOnePointZero(n)) {
    n = '100%';
  }
  let percentageValue: number;
  const processPercent = isPercentage(n);
  percentageValue = mathMin(max, mathMax(0, parseFloat(n)));

  // Automatically convert percentage into number
  if (processPercent) {
    percentageValue = (percentageValue * max) / 100;
  }

  // Handle floating point rounding errors
  if (Math.abs(percentageValue - max) < 0.000001) {
    return 1;
  }

  // Convert into [0, 1] range if it isn't already
  return (percentageValue % max) / max;
}
